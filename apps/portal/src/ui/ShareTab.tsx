 ...
 *
 * Under the link sits the context card (AiContextCard). Saving it while a
 * link is live replaces the link's text in place, so a URL already pasted
 * into an assistant serves the newer page.
 *
...
interface Props {
  ...
  onSaveContext: (ctx: AiContext) => Promise<void>;
}
...
export default function ShareTab({ reports, trends, context, onSaveContext }: Props) {
  const text = useMemo(() => buildAiShare(reports, trends, context), [reports, trends, context]);
  ...
  /** The account first; then, if a link is live, its page gets the new text
   *  under the same URL. A failed refresh is not a failed save — the next
   *  mint carries the context anyway. */
  async function saveContext(next: AiContext) {
    await onSaveContext(next);
    if (!remote || !alive(remote.expiresAt)) return;
    ...
    try {
      ...
      /* the link keeps its older text until the next mint */
    }
  }

  const contextCard = <AiContextCard value={context} onSave={saveContext} />;
        ...
        <div className="card ai-share">
          ...
          <details style={{ marginTop: 14 }}>
            ...
            <p className="muted" style={{ margin: "8px 0 6px" }}>
              Prostý text, bez jména a bez PDF{!isEmptyAiContext(context) ? " — a to, co jste o sobě vyplnili" : ""}.{" "}
