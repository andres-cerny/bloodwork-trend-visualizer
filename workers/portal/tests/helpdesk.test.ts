...
function fakeD1(t: Tables): D1Database {
  const run = (sql: string, a: unknown[]): { results: unknown[]; changes: number } => {
    switch (sql) {
      ...
      // The document slot the extract proxy takes first (src/allowance.ts) —
...
const A = { id: "u-a", email: "a@example.com", created_at: "2026-01-01T00:00:00Z", settings: null, session_epoch: 0, budget_usd: null, doc_allowance: 5, doc_used: 0 };
...
beforeEach(() => {
  tables = { users: [{ ...A }], messages: [], events: [], documents: [] };
  sent = [];
  fetched = [];
  turnstileOk = true;
  env = {
...
describe("the events table", () => {
  ...
  it("gets a row for an upload the extractor turned down, on the buffered and the streamed path", async () => {
    const headers = { ...(await cookie("u-a")), "content-type": "application/json", "x-document": "doc-1" };
    await worker.fetch(new Request("https://portal/api/documents", { method: "POST", headers, body: JSON.stringify({ id: "doc-1" }) }), env);
    env.EXTRACT = {
