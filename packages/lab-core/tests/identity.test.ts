...
import type { LabReport } from "../src/models";
...
describe("rodné číslo normalization", () => {
  ...
  it("compares a number the birth-date parser would reject", () => {
    // 991350/0000 has month 13 and day 50: parseRodneCislo returns null. It is
    // still the same nine or ten digits on both reports, and refusing to
    // compare it would disable the guard on the scans that need it most.
    expect(normalizeRodneCislo("991350/0000")).toBe("9913500000");
    expect(
      compareIdentities({ name: null, id: "991350/0000" }, { name: null, id: "9913500000" }),
    ).toBe("same");
  });
});
...
describe("comparing two identities", () => {
  ...
  it("falls back to the name when an id is present but unreadable", () => {
    // Rule 1 in the file header: a misread number must not decide anything.
    expect(
      compareIdentities({ name: "Jan Novák", id: "8001" }, { name: "Novák Jan", id: "800101/0011" }),
    ).toBe("same");
  });
...
describe("listing who is loaded", () => {
  ...
  it("skips reports carrying no identity at all", () => {
    expect(distinctIdentities([report(null, "  ", "a")])).toEqual([]);
  });
});
...
describe("describing an identity for the dialog", () => {
  ...
  it("agrees with hasIdentity about what is worth showing", () => {
    expect(hasIdentity({ name: "  ", id: "" })).toBe(false);
    expect(hasIdentity({ name: null, id: "800101/0011" })).toBe(true);
  });
});
