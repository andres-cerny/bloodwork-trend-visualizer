 ...
 *
 * This is deliberately not part of `review.ts`. That file answers "can this
 * *reading* be trusted", one `Measurement` at a time, and every consumer of it
 * is a chip or a filter over rows. The question here is asked once per
 * document, before any of its rows exist as far as the app is concerned, and
 * its answer is a gate rather than a mark. Folding it in would widen `Review`
 * to a union that most of its callers could not act on.
 */
 ...
 *
 * Not `parseRodneCislo`, which additionally decodes the birth date and would
 * reject an impossible one. That is the right check when the number is about
 * to be *believed* — an age printed beside a name. Here it is only ever
 * compared against another number, and two reports printing the same
 * mistranscribed digits are still the same patient. Routing this through the
 * stricter parser would quietly disable the guard on exactly the scans whose
 * headers transcribe worst.
 */
...
/** Is there anything here a human could be shown? */
export function hasIdentity(i: Identity): boolean {
  return Boolean(i.name?.trim() || i.id?.trim());
}

/** One line naming an identity, for the dialog. */
...
/**
 * Named `IdentityVerdict` rather than `Verdict`: `mapping.ts` already exports
 * that name for a different judgement, and both reach callers through the
 * package root.
 */
 ...
 */
export function compareIdentities(a: Identity, b: Identity): IdentityVerdict {
