...
| **A first login with several PDFs picked at once: Souhrn opened on the first report and rearranged itself as the others landed** (Ondřej, on his own account). The screen switched on `reports.length > 0`, which the first stored report satisfied; and the switch moved the upload card into the Reporty panel, which remounted the queue and dropped its log and progress bars — the rest of the pick landed silently | a screen switching on the first of a set | The files picked together are one batch (`apps/portal/src/lib/batch.ts`): counted in `UploadFlow.tsx` where a file enters and where it ends (one log entry per file, stored, failed or skipped), and while a batch runs on an account that had no reports, `Portal.tsx` holds the switch until the last file settles — one switch, every report in it. A failed file does not hold the others. Under the drop target, while the hold is on: „Souhrn se otevře až po přečtení všech N souborů." An account with reports keeps today's behaviour. The panels are mounted from the first render (Reporty shown, the rest empty) so the queue survives the switch and the failed file stays listed with its message | `apps/portal/tests/batch.test.ts` (three fail with the hold disabled) and `uploadFlow.test.tsx` (the queue steps the batch in a DOM); „the first upload, several files at once" in `tests/e2e/upload-portal.e2e.ts` holds the second read and looks: no strip, one report listed, the line — both fail with the hold disabled; „první přihlášení" and „první nahrání (dávka běží)" in `audit-portal.e2e.ts` |
