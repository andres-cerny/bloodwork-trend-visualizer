...
describe("spend ledger", () => {
  ...
  it("freezes both AI routes once the ceiling is reached", async () => {
    ...
    // The agent is NOT frozen by extraction's spend, and that is the point of
    // keying the ledger per capability: two demos used to share one fuse, so a
    // batch of uploads could take the chat down with it. Each now has its own
    // ceiling and its own failure.
  ...
  it("freezes the agent on the agent's own spend", async () => {
    const env = makeEnv({ BUDGET_USD_LIMIT: "0.02" });
    const s = await mintSession(SECRET, 600, 12);
    ...
    // ...while extraction, which spent nothing, still works.
    expect((await worker.fetch(post("/api/extract", { rowsText: "x" }, s), env)).status).toBe(200);
  });
...
/**
 * The chat route, which until now had no tests of its own.
 *
 * The old suite touched it twice, both times to assert a shared gate — that it
 * does not burn a page allowance, and that it freezes with the ledger. Nothing
 * covered what it returns. Because the harness answered every call with a
 * tool_use block, the text filter produced an empty string and the tests passed
 * regardless. It carries two apps and a tool loop now.
 */
